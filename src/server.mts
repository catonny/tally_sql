...
import { SyncConfig, testOdbcConnection, testPostgresConnection } from './odbc-sync.mjs';
...
function runSyncProcess(configObj: SyncConfig): void {
...
function readJson(body: string): any {
...
const httpServer = http.createServer((req, res) => {
    ...
    req.on('end', async () => {
        try {
            ...
            if (req.url === '/sync') {
                const configObj = readJson(reqContent) as SyncConfig;
                if (isSyncRunning) {
                    res.statusCode = 200;
                    res.setHeader('Content-Type', 'text/plain');
