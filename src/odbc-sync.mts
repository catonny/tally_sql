import { Pool } from 'pg';
import odbc, { ColumnsEntry, Result, TableEntry } from 'odbc';
...
function normalizeTablesList(lst?: string[]): string[] {
    if (!lst) return [];
    return lst
        .map((item) => item.trim())
...
export function buildOdbcConnectionString(config: OdbcConnectionConfig): string {
...
export async function testOdbcConnection(config: OdbcConnectionConfig): Promise<string[]> {
    ...
    try {
        ...
        const tables: string[] = [];
        for (const row of result as Result<TableEntry>) {
...
export async function testPostgresConnection(config: PostgresConnectionConfig): Promise<void> {
    const pool = new Pool({
        host: config.host,
...
function schemaQualifiedName(schema: string, table: string): string {
    return `${quoteIdentifier(schema)}.${quoteIdentifier(table)}`;
}

function mapOdbcTypeToPg(typeCode?: number): string {
    if (typeCode === undefined || typeCode === null) return 'TEXT';
...
function inferPgTypeFromValue(value: unknown): string {
    ...
    if (value instanceof Buffer) return 'BYTEA';
    return 'TEXT';
}

function prepareValueForPg(value: any, pgType: string): any {
...
class PostgresTarget {
    ...
    async ensureTable(tableName: string, columns: ColumnDefinition[]): Promise<void> {
        ...
        if (existingColumns.rowCount === 0) {
            ...
            const existingSet = new Set(existingColumns.rows.map((row) => (row.column_name as string).toLowerCase()));
            for (const column of columns) {
                if (!existingSet.has(column.name.toLowerCase())) {
                    await this.pool.query(
                        `ALTER TABLE ${schemaTable} ADD COLUMN ${quoteIdentifier(column.name)} ${column.pgType}`
                    );
                }
            }
        }
    ...
    async replaceTableData(tableName: string, columns: ColumnDefinition[], rows: any[], truncate: boolean): Promise<number> {
...
export class OdbcReplicator {
    ...
    private odbcConnection?: odbc.Connection;
    private postgresTarget?: PostgresTarget;
    private running = false;

    constructor(config: SyncConfig, logger: (message: string) => void = console.log) {
        this.config = {
            ...config,
            odbc: {
                ...config.odbc,
    ...
    private log(message: string): void {
        ...
        if (this.logger) this.logger(formatted);
    }
    ...
    private async getTablesToProcess(): Promise<string[]> {
        if (this.config.odbc.tables && this.config.odbc.tables.length > 0) {
            return this.config.odbc.tables;
        }
        if (!this.odbcConnection) throw new Error('ODBC connection is not initialised.');
    ...
    private convertColumnDefinitions(metadata: Result<ColumnsEntry>, sampleRow?: any): ColumnDefinition[] {
    ...
    private async fetchRows(tableName: string): Promise<any[]> {
        if (!this.odbcConnection) throw new Error('ODBC connection is not initialised.');
    ...
    private async cloneTable(tableName: string): Promise<void> {
        if (!this.odbcConnection || !this.postgresTarget) throw new Error('Connections are not initialised.');
